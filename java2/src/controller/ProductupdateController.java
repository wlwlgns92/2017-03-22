...
public class ProductupdateController implements Initializable{
	...
	txtpprice.setText(product.getP_price()+"");		 // 4. 클릭된 객체 제품가격
		Image image = new Image(product.getP_img()); // 5. 클릭된 객체내 이미지 경로
		pimg.setImage(image);;						 // * 이미지 세팅
		lblimgpath.setText(product.getP_img());		 // * 이미지 경로
		pimage = product.getP_img(); // 수정이 없을경우 기존 이미지 사용
		
		// 제품에 저장된 카테고리별 라디오 버튼 클릭 세팅 [ setSelected(true) ]
	   ...
	   @FXML
	    ...
	    void imgadd(ActionEvent event) {
	    	...
	    	fileChooser.getExtensionFilters().add(new ExtensionFilter("그림파일 : ", "*jpg", "*png", "*gif"));
