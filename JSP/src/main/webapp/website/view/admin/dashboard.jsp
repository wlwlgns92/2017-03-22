...
<body>
		...
		%>
		<div class="container">
			<form action="dashboard.jsp" method="get" class="col-md-5 offset-3">
			...
			<table class="table">
				<tr>
					<th>이미지 </th> <th>제품번호</th> <th>제품명</th> <th>제품 가격</th> <th> 카테고리</th>
				...
				<%for(Product product : products) { %>
					<tr>
						...
						<td> <a href="productupdate.jsp?p_num=<%=product.getP_num() %>" > 
							 ...
							 <a href="../../controller/productactivecontroller.jsp?p_num=<%=product.getP_num() %>" > <button class="btn-outline-info" onclick="activeupdate(<%=product.getP_num()%>)">상태변경</button> </a> 
						</td>
				<%} %>
